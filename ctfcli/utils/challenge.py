...
def get_challenge_details(challenge_id):
    ...
    challenge["flags"] = [
        f["content"]
        if f["type"] == "static" and (f["data"] == None or f["data"] == "")
        else {"content": f["content"], "type": f["type"], "data": f["data"]}
        for f in flags
    ]
    ...
    hints = r.json()["data"]
    challenge["hints"] = [
        {"content": h["content"], "cost": h["cost"]} if h["cost"] > 0 else h["content"]
        for h in hints
    ]
    ...
    if len(requirements) > 0:
        ...
        challenge["requirements"] = [
            c["name"] for c in challenges if c["id"] in requirements
        ]
            ...
            for f in remote_challenge["files"]:
                ...
                if f_base not in local_files and _verify_new_files:
                    ...
                    if verify_files:
                        ...
                        local_file = Path(
                            challenge.directory, local_files[f_base]
                        ).read_bytes()
...
def pull_challenge(
    challenge, ignore=(), update_files=False, create_files=False, create_defaults=False
):
    """
    ...
    try:
        verify_challenge(
            challenge,
            ignore=ignore,
            verify_files=update_files,
            verify_defaults=create_defaults,
            _verify_new_files=create_files,
        )
    ...
    remote_cleaned_files = [
        f.split("/")[-1].split("?token=")[0] for f in remote_details["files"]
    ]
    challenge["files"] = [
        f for f in challenge.get("files", []) if Path(f).name in remote_cleaned_files
    ]
