...
def get_challenge_details(challenge_id):
    ...
    r.raise_for_status()
    flags = r.json()["data"]
    challenge["flags"] = [f["content"] if f["type"] == "static" and (f["data"] == None or f["data"] == "") else {
        "content": f["content"], "type": f["type"], "data": f["data"]} for f in flags]
    ...
    r.raise_for_status()
    hints = r.json()["data"]
    challenge["hints"] = [{"content": h["content"], "cost": h["cost"]}
                          if h["cost"] > 0 else h["content"] for h in hints]
    ...
    if len(requirements) > 0:
        ...
        challenge["requirements"] = [c["name"]
                                     for c in challenges if c["id"] in requirements]
        ...
        if key == "files":
            ...
            for f in remote_challenge["files"]:
                ...
                if f_base not in local_files and _verify_new_files:
                    raise Exception(
                        f"Remote challenge has file {f_base} that is not present locally")
                else:
                    if verify_files:
                        ...
                        local_file = Path(challenge.directory,
                                          local_files[f_base]).read_bytes()
                        if remote_file != local_file:
                            raise Exception(
                                f"Remote challenge file {f_base} does not match local file")
            ...
            raise Exception(
                f"Field {key} in challenge.yml does not match remote challenge")
...
def pull_challenge(challenge, ignore=(), update_files=False, create_files=False, create_defaults=False):
    ...
    try:
        verify_challenge(challenge, ignore=ignore, verify_files=update_files,
                         verify_defaults=create_defaults, _verify_new_files=create_files)
        click.secho(
            f"Challenge {challenge['name']} is already up to date", fg="green")
        return
    except Exception:
        pass
    ...
    remote_cleaned_files = [f.split("/")[-1].split('?token=')[0]
                            for f in remote_details["files"]]
    challenge["files"] = [f for f in challenge.get(
        "files", []) if Path(f).name in remote_cleaned_files]
